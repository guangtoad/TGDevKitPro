...
@implementation NSString (TGRegex)
...
/**
 ...
 @return 匹配结果
 */
- (BOOL) regexByStr:(NSString * _Nullable)regex{
    if (!regex || 0 == regex.length || 0 == self.length) {
        return false;
    }
...
- (NSString * _Nullable) subStringWithRegex:(NSString * _Nullable)regexStr{
    NSRange range = [self rangeOfString:regexStr options:NSRegularExpressionSearch];
