...
- (BOOL) openScheme:(NSString * __nullable)scheme  viewController:(UIViewController *)viewController  params:(NSDictionary<NSString *,id> * __nullable)params  handler:(TGRouterHandler * __nullable)handler completion:(void (^ __nullable)(void))completion{
    BOOL animated = true;
    // = [[params objectForKey:TGRouter_Key_Animated] boolValue];
...
- (BOOL) openScheme:(NSString * __nullable)scheme host:(NSString * __nullable)host path:(NSString * __nullable)path params:(NSDictionary<NSString *,id> * __nullable)params  handler:(TGRouterHandler * __nullable)handler completion:(void (^ __nullable)(void))completion{
    if ([TGRouter_Scheme_Push isEqualToString:scheme] || [TGRouter_Scheme_Present isEqualToString:scheme]) {
        ...
        return [self openScheme:scheme viewController:viewController params:params handler:handler completion:completion];
    }
    return false;
}
