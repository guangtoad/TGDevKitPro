...
public class BSCTool {
    ...
    public BSCTool(){
//        BSCConfig bscConfig = TGConfig.getInstance().bscConfig;
//        this.beyondComparePath = bscConfig.beyondComparePath;
//        this.left = bscConfig.left;
//        this.right = bscConfig.right;
//        this.filter = bscConfig.filter;
//        this.out = bscConfig.out;
//        this.dirDiffTxtName = bscConfig.dirDiffTxtName;
//        this.dirDiffSprictName = bscConfig.dirDiffSprictName;
//        this.fileDiffSprictName = bscConfig.fileDiffSprictName;
    }
    ...
    public void readDirDiffTxt(String txtPath) throws Exception{
        ...
        try {
            ...
            //TGConfig.getInstance().bscConfig;

            System.out.println("File:" + "==========读取开始==========");
            while ((tempString = reader.readLine()) != null) {

                System.out.println("File:" + tempString);

                if (beginDiff) {
                    if (Pattern.matches(config.diffLineRegex,tempString)) {
                        String diff = tempString.replaceAll(config.diffLineSuffixRegex,"");
                        if (diff.length() > 0) {
                            if (null == leftBase){
                                System.out.println("没找到:" + "左边基础路径");
                                continue;
                            }
                            if (null == rightBase){
                                System.out.println("没找到:" + "右边基础路径");
                                continue;
                            }
                            ...
                            System.out.println("准备 写入 diff:" + diff);
                            ...
                            System.out.println("写入 diff:" + diff);
                            System.out.println(outString);
                        }
                    }
                }
                else if (Pattern.matches(config.diffBeginRegex, tempString)){
                    beginDiff = true;
                    System.out.println("开始不同了");
                }
                else if (Pattern.matches(config.diffLeftRegex, tempString)) {
                    leftBase = new File(tempString.replaceAll(config.diffLeftPrefixRegex,""));
                    System.out.println("左边:" + leftBase.getCanonicalPath());
                }
                else if (Pattern.matches(config.diffRigtRegex, tempString)){
                    rightBase = new File(tempString.replaceAll(config.diffRigtPrefixRegex,""));
                    System.out.println("左边:" + rightBase.getCanonicalPath());
                }
            }
            System.out.println("File:" + "==========读取结束==========");
    ...
    public static void main(String[] args) {
        ...
        try {
            BSCTool bscTool = new BSCTool();
            bscTool.abd();
            bscTool.readDirDiffTxt("/Users/toad/Desktop/testby/diff/BSCTool/out/dir.diff.txt");
        }
        catch (Exception e){
