...
- (id) init{
    if (self = [super init]) {
        ...
        /** 按钮 */
        _btnTitleFont = [UIFont systemFontOfSize:18.0f];
        /** 按钮背景色 */
...
- (__kindof UIButton *) cancelBtnByTitle:(NSString *)title{
    ...
    if ([(TGSheetViewStyle *)self.style cancelHeight] == 0) {
        [(TGSheetViewStyle *)self.style setCancelHeight:self.styel.rowHegiht];
    }
    frame.size.height = self.styel.cancelHeight > 0 ? self.styel.cancelHeight :48;
...
- (id) initWithStyle:(TGSheetViewStyle *)style{
    ...
    if (self = [self initWithFrame:frame]) {
        ...
        self.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];;//self.styel.maskBackgrouColor;
