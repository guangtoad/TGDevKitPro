...
-(void)initUIView{
    ...
    contentTextField.frame = CGRectMake(30, (([[[UIDevice currentDevice] systemVersion] floatValue]>=7.0) ? 65 : 45)+30, [[UIScreen mainScreen] bounds].size.width-60, 45);
//    contentTextField.backgroundColor = ColorRGB(179, 143, 0);
    contentTextField.placeholder = @"  Content";
    contentTextField.font = [UIFont fontWithName:@"Arial" size:16];
    [self.view addSubview:contentTextField];
    
    hTextField = [[UITextField alloc] init];
    hTextField.frame = CGRectMake(30+([[UIScreen mainScreen] bounds].size.width-70)/2+10, contentTextField.frame.origin.y+contentTextField.frame.size.height+20, ([[UIScreen mainScreen] bounds].size.width-70)/2, 45);
    hTextField.keyboardType = UIKeyboardTypePhonePad;
//    hTextField.backgroundColor = ColorRGB(179, 143, 0);
    hTextField.placeholder = @"  Height";
    hTextField.font = [UIFont fontWithName:@"Arial" size:16];
    [self.view addSubview:hTextField];
    
    wTextField = [[UITextField alloc] init];
    wTextField.frame = CGRectMake(30, contentTextField.frame.origin.y+contentTextField.frame.size.height+20, ([[UIScreen mainScreen] bounds].size.width-70)/2, 45);
    wTextField.keyboardType = UIKeyboardTypePhonePad;
//    wTextField.backgroundColor = ColorRGB(179, 143, 0);
    wTextField.placeholder = @"  Width";
    wTextField.font = [UIFont fontWithName:@"Arial" size:16];
    [self.view addSubview:wTextField];
    
    seleteTypeButton = [UIButton buttonWithType:0];
    seleteTypeButton.frame = CGRectMake(30, wTextField.frame.origin.y+wTextField.frame.size.height+20, [[UIScreen mainScreen] bounds].size.width-60, 45);
    ...
    seleteTypeButton.titleLabel.font = [UIFont fontWithName:@"Arial" size:16];
//    seleteTypeButton.titleLabel.textColor = ColorRGB(0, 86, 179);
    ...
    makeSureButton.frame = CGRectMake(30, seleteTypeButton.frame.origin.y+seleteTypeButton.frame.size.height+20, [[UIScreen mainScreen] bounds].size.width-60, 45);
    makeSureButton.backgroundColor = [UIColor blueColor];
    [makeSureButton setTitle:@"Confirm" forState:UIControlStateNormal];
    makeSureButton.titleLabel.font = [UIFont fontWithName:@"Arial" size:16];
...
//    blurView.backgroundColor = ColorRGBA(0.0, 0.0, 0.0, 0.6);
    ...
    pickerBgView.frame = CGRectMake(0, [[UIScreen mainScreen] bounds].size.height-240, [[UIScreen mainScreen] bounds].size.width, 240);
...
//    [cancelButton setTitleColor:ColorRGB(0, 111, 222) forState:UIControlStateNormal];
    ...
    lineView.frame = CGRectMake(0, 39, [[UIScreen mainScreen] bounds].size.width, 1);
//    lineView.backgroundColor = ColorRGB(230, 230, 230);
    ...
    sureButton.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width-70, 0, 70, 40);
...
//    [sureButton setTitleColor:ColorRGB(0, 111, 222) forState:UIControlStateNormal];
    ...
    seletePicker = [[UIPickerView alloc]initWithFrame:CGRectMake(0, 40, [[UIScreen mainScreen] bounds].size.width, 200)];
...
-(void)qdButtonAction:(id)sender{
    ...
    if ([self hasBlankSpace:wTextField.text] == YES) {
        ...
        wTextField.text = [NSString stringWithFormat:@"%d", (int)([[UIScreen mainScreen] bounds].size.width-60)];
    }
    if ([self hasBlankSpace:hTextField.text] == YES) {
        ...
        if ([seleteCodeTypeString isEqualToString:@"AztecCodeGenerator"] || [seleteCodeTypeString isEqualToString:@"QRCodeGenerator"] || [seleteCodeTypeString isEqualToString:@"DataMatrixCodeGenerator"]) {//Need to generate square code graph
            hTextField.text = [NSString stringWithFormat:@"%d", (int)([[UIScreen mainScreen] bounds].size.width-60)];
        }else{
            hTextField.text = [NSString stringWithFormat:@"%d", (int)([[UIScreen mainScreen] bounds].size.width-60)/2];
        }
    }
    ...
    if (w>[[UIScreen mainScreen] bounds].size.width-60) {
        w = [[UIScreen mainScreen] bounds].size.width-60;
    ...
    if (h>[[UIScreen mainScreen] bounds].size.height-upHeight) {
        h = [[UIScreen mainScreen] bounds].size.height-upHeight-50;
    ...
    imgView = [[UIImageView alloc] initWithFrame:CGRectMake(([[UIScreen mainScreen] bounds].size.width-w)/2, upHeight+([[UIScreen mainScreen] bounds].size.height-upHeight)/2-h/2, w, h)];
    imgView.image = image;
